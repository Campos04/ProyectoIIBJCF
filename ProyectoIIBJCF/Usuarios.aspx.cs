    ...
    public partial class Usuarios : SecurePage
    {
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlCommand command = DbHelper.CreateStoredProcedureCommand("sp_Usuarios_Agregar", connection))
                {
        ...
        protected void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            {
                using (SqlConnection connection = DbHelper.CreateConnection())
                using (SqlCommand command = DbHelper.CreateStoredProcedureCommand("sp_Usuarios_Modificar", connection))
                {
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                using (SqlConnection connection = DbHelper.CreateConnection())
                using (SqlCommand command = DbHelper.CreateStoredProcedureCommand("sp_Usuarios_Eliminar", connection))
                {
                    command.Parameters.Add("@UsuarioID", SqlDbType.Int).Value = userId;
                    connection.Open();
                    command.ExecuteNonQuery();
            ...
            catch (SqlException) { ShowMessage("No se pudo borrar el usuario. Revisa si tiene equipos asociados o si es el usuario de acceso.", true); }
        }
        ...
        private void LoadUserById(int userId)
        {
            ...
            {
                using (SqlConnection connection = DbHelper.CreateConnection())
                using (SqlCommand command = DbHelper.CreateStoredProcedureCommand("sp_Usuarios_Consultar", connection))
                {
                    command.Parameters.Add("@UsuarioID", SqlDbType.Int).Value = userId;
                    connection.Open();
